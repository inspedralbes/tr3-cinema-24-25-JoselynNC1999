<template>
      ...
      <section class="py-16 px-6 relative overflow-hidden">
        <div class="container mx-auto relative z-10">
          ...
          <div class="flex items-center text-blue-300 mb-6">
            ...
            <span class="mx-2">/</span>
            <router-link :to="`/pelicula/${orderId}`" class="hover:text-white transition-colors">{{ movieTitle }}</router-link>
            <span class="mx-2">/</span>
            <router-link :to="`/seleccio-butaques/${orderId}`" class="hover:text-white transition-colors">Selecció Butaques</router-link>
            <span class="mx-2">/</span>
                  ...
                  <div class="md:w-2/3">
                    <div class="bg-blue-900/50 rounded-xl p-6 mb-6">
                      ...
                      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <p class="font-semibold">{{ movieFormat }}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="bg-blue-900/50 rounded-xl p-6 mb-6">
                      ...
                      <div class="space-y-3 mb-4">
                        <div v-for="(seat, index) in selectedSeats" :key="index" class="flex justify-between items-center">
                          <div class="flex items-center">
                            <div class="w-8 h-8 rounded-lg flex items-center justify-center mr-2"
                              :class="seat.type === 'VIP' ? 'bg-purple-600' : 'bg-blue-600'">
                              <span class="font-bold text-sm">{{ seat.row }}{{ seat.number }}</span>
                            </div>
                            <span>Butaca {{ seat.type === 'VIP' ? 'VIP' : 'Estàndard' }}</span>
                          </div>
                          <div class="text-right">
                            <span class="font-bold">{{ formatPrice(seat.price) }}</span>
                            <span class="text-xs text-blue-300 block" v-if="isDiscountApplied">Amb descompte 2x1</span>
                          </div>
                        </div>
                      </div>
  ...
  const orderId = ref(route.params.id || '12345');
  const orderCode = ref('CINP' + Math.floor(Math.random() * 10000).toString().padStart(4, '0'));
  const movieTitle = ref('Aventures Galàctiques');
  const movieGenre = ref('Ciència Ficció');
  const movieDuration = ref('120 min');
  const movieFormat = ref('Digital 2D');
  const sessionDate = ref('17 de Març, 2025');
  const sessionTime = ref('19:30h');
  const theaterRoom = ref('Sala 3');
  const isDiscountApplied = ref(true); // Wednesday 2x1 promo
  
  // Sample selected seats
  const selectedSeats = ref([
    { row: 'F', number: '12', type: 'Standard', price: 9.50 },
    { row: 'F', number: '13', type: 'Standard', price: 0 }, // Free with 2x1 promo
    { row: 'G', number: '8', type: 'VIP', price: 12.50 },
  ]);
  
  // Calculate total price
  const totalPrice = ref(22.00); // 9.50 + 0 (2x1) + 12.50
  ...
  onMounted(() => {
    // You could fetch order data here based on the route params
